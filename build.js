...
// Short SHA of the commit being built. Stamped into the bundle so a deploy can
// be verified from the served file rather than trusted from a green push.
// Cloudflare Pages exposes CF_PAGES_COMMIT_SHA; git is the local fallback.
const BUILD_ID = (() => {
...
async function buildTypeScript() {
  ...
  try {
    await esbuild.build({
      ...
      minify: isProduction,
      define: define,
      // GSAP is loaded by Webflow, not bundled — bundling it too would give the
      // page two GSAP instances and register plugins against the wrong one.
      // Component code reads window.gsap through src/lib/gsap.ts instead.
...
/**
 * Emit Cloudflare Pages headers.
 *
 * Production caches for a minute so the edge stays fast while a push is still
 * live within about a minute — no version bump, no Webflow edit. Staging is
 * never cached, so a preview always reflects the latest push.
 */
function buildHeaders() {
  const branch = process.env.CF_PAGES_BRANCH || 'local';
  const cache = branch === 'main' ? 'public, max-age=60, must-revalidate' : 'no-cache';
...
`
  ...
  console.log(`✓ Wrote dist/_headers (branch: ${branch}, cache: ${cache})`);
}
...
async function build() {
  ...
  // Step 3b: Cache + CORS headers for Cloudflare Pages
  buildHeaders();
